...
def session_fixture():
    ...
    with Session(engine) as session:
        ...
        p = Proposicao(
            id="123", tipo="PL", numero="1", ano=2024, autor="Test", 
            status="Test", ementa="Test", data_apresentacao="2024-01-01", 
            data_ultima_movimentacao="2024-01-01", orgao_atual="Test", tags=[]
        )
        session.add(p)
        session.commit()
        yield session
...
def test_salvar_lote_insere_multiplos(session: Session):
    ...
    t1 = Tramitacao(proposicao_id="123", data_hora="2024-01-01T10:00", sequencia=1, sigla_orgao="OR1", descricao_tramitacao="D1")
    t2 = Tramitacao(proposicao_id="123", data_hora="2024-01-01T11:00", sequencia=2, sigla_orgao="OR1", descricao_tramitacao="D2")
...
def test_deletar_por_proposicao_limpa_apenas_especifica(session: Session):
    ...
    p2 = Proposicao(
        id="456", tipo="PL", numero="2", ano=2024, autor="Test", 
        status="Test", ementa="Test", data_apresentacao="2024-01-01", 
        data_ultima_movimentacao="2024-01-01", orgao_atual="Test", tags=[]
    )
    session.add(p2)
    t2 = Tramitacao(proposicao_id="456", data_hora="2024-01-01T10:00", sequencia=1, sigla_orgao="OR1", descricao_tramitacao="D1")
    session.add(t2)
    session.commit()
