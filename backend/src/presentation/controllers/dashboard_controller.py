...
@router.get("/dashboard/grafico-comissao", response_model=List[DadosGraficoComissaoResponse])
def obter_dados_comissao(service: DashboardService = Depends(get_dashboard_service)):
    return service.obter_dados_comissao()

@router.get("/dashboard/grafico-status", response_model=List[DadosGraficoStatusResponse])
