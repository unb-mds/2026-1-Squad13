...
class SQLDashboardRepository:
    ...
    def obter_estoque_fases(self, filtros: dict | None = None) -> list[dict]:
        ...
        statement = select(
            ...
            FaseAnaliticaModel, PeriodoFaseModel.fase_analitica_id == FaseAnaliticaModel.id
        ).where(
            PeriodoFaseModel.eh_fase_atual == True
        )
        
        if filtros:
            tipo = filtros.get("tipo")
            if tipo:
                statement = statement.where(func.lower(PeriodoFaseModel.tipo_proposicao) == tipo.lower())
            rito = filtros.get("rito")
            if rito:
                statement = statement.where(func.lower(PeriodoFaseModel.rito) == rito.lower())
    ...
    def obter_mediana_handoff(self, filtros: dict | None = None) -> dict:
        ...
        stmt_count = select(func.count(PeriodoFaseModel.proposicao_id)).join(
            FaseAnaliticaModel, PeriodoFaseModel.fase_analitica_id == FaseAnaliticaModel.id
        ).where(
            PeriodoFaseModel.eh_fase_atual == True,
        ...
        if filtros:
            tipo = filtros.get("tipo")
            if tipo:
                stmt_count = stmt_count.where(func.lower(PeriodoFaseModel.tipo_proposicao) == tipo.lower())
            rito = filtros.get("rito")
            if rito:
                stmt_count = stmt_count.where(func.lower(PeriodoFaseModel.rito) == rito.lower())
        total_em_transito = self.session.exec(stmt_count).first() or 0
        
        stmt_durations = select(
        ...
        if filtros:
            tipo = filtros.get("tipo")
            if tipo:
                stmt_durations = stmt_durations.where(func.lower(PeriodoFaseModel.tipo_proposicao) == tipo.lower())
            rito = filtros.get("rito")
            if rito:
                stmt_durations = stmt_durations.where(func.lower(PeriodoFaseModel.rito) == rito.lower())
    ...
    def obter_tempo_por_fase(self, filtros: dict | None = None) -> list[dict]:
        ...
        for code, data in grouped.items():
            mediana = int(statistics.median(data["duracoes"])) if data["duracoes"] else 0
            resultado.append({
                "fase": data["nome"],
                "codigoFase": code,
    ...
    def obter_qualidade_base(self, filtros: dict | None = None) -> dict:
        statement = select(
            ...
            func.sum(case((and_(ProposicaoModel.tipo.isnot(None), ProposicaoModel.tipo != ""), 1), else_=0)).label("tipo"),
            func.sum(case((and_(ProposicaoModel.numero.isnot(None), ProposicaoModel.numero != ""), 1), else_=0)).label("numero"),
            func.sum(case((ProposicaoModel.ano.isnot(None), 1), else_=0)).label("ano"),
            func.sum(case((and_(ProposicaoModel.ementa.isnot(None), ProposicaoModel.ementa != ""), 1), else_=0)).label("ementa"),
            func.sum(case((and_(ProposicaoModel.autor.isnot(None), ProposicaoModel.autor != ""), 1), else_=0)).label("autor"),
            func.sum(case((and_(ProposicaoModel.orgao_origem.isnot(None), ProposicaoModel.orgao_origem != ""), 1), else_=0)).label("orgao_origem"),
            func.sum(case((and_(ProposicaoModel.status.isnot(None), ProposicaoModel.status != ""), 1), else_=0)).label("status"),
            func.sum(case((and_(ProposicaoModel.orgao_atual.isnot(None), ProposicaoModel.orgao_atual != ""), 1), else_=0)).label("orgao_atual"),
            func.sum(case((and_(ProposicaoModel.data_apresentacao.isnot(None), ProposicaoModel.data_apresentacao != ""), 1), else_=0)).label("data_apresentacao"),
            func.sum(case((and_(ProposicaoModel.data_ultima_movimentacao.isnot(None), ProposicaoModel.data_ultima_movimentacao != ""), 1), else_=0)).label("data_ultima_movimentacao"),
            func.sum(case((and_(ProposicaoModel.link_oficial.isnot(None), ProposicaoModel.link_oficial != ""), 1), else_=0)).label("link_oficial"),
            func.sum(case((and_(ProposicaoModel.regime_tramitacao.isnot(None), ProposicaoModel.regime_tramitacao != ""), 1), else_=0)).label("regime_tramitacao")
        )
        if filtros:
        ...
        total = row.total
        campos = [
            row.tipo, row.numero, row.ano, row.ementa, row.autor, 
            row.orgao_origem, row.status, row.orgao_atual, 
            row.data_apresentacao, row.data_ultima_movimentacao, 
            row.link_oficial, row.regime_tramitacao
        ]
        
        soma_preenchidos = sum(c if c is not None else 0 for c in campos)
        max_possivel = total * len(campos)
        completude = (soma_preenchidos / max_possivel * 100) if max_possivel > 0 else 0.0
