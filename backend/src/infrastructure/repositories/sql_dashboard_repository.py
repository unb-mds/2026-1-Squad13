...
class SQLDashboardRepository:
    ...
    def _aplicar_filtros(self, statement: Any, filtros: Optional[Dict]) -> Any:
        ...
        if orgao_origem:
            statement = statement.where(func.lower(ProposicaoModel.orgao_origem) == orgao_origem.lower())
        if data_inicio:
    ...
    def obter_metricas_gerais(self, filtros: Optional[Dict]) -> Dict:
        ...
        stmt = select(
            ...
            func.coalesce(func.avg(ProposicaoModel.tempo_total_dias), 0).label("tempo_medio"),
            func.sum(case((and_(ProposicaoModel.tempo_total_dias > 180, ProposicaoModel.data_encerramento.is_(None)), 1), else_=0)).label("com_atraso"),
            func.sum(case((status_agrupado == "Aprovada/Sancionada", 1), else_=0)).label("aprovadas"),
            func.sum(case((status_agrupado == "Em tramitação", 1), else_=0)).label("em_tramitacao"),
            func.sum(case((status_agrupado == "Rejeitada/Arquivada", 1), else_=0)).label("rejeitadas")
        )
        ...
        ).where(ProposicaoModel.orgao_atual.isnot(None), ProposicaoModel.tempo_total_dias.isnot(None))
        stmt_orgao = self._aplicar_filtros(stmt_orgao, filtros)
        stmt_orgao = stmt_orgao.group_by(ProposicaoModel.orgao_atual).order_by(func.avg(ProposicaoModel.tempo_total_dias).desc()).limit(1)
        ...
        return {
            ...
            "comissaoMaiorTempo": pior_orgao_row.orgao_atual if pior_orgao_row else "N/A",
            "comissaoMaiorTempoMedia": int(pior_orgao_row.media_tempo or 0) if pior_orgao_row else 0,
        }
    ...
    def obter_dados_tipo(self, filtros: Optional[Dict]) -> List[Dict]:
        ...
        return [{"tipo": row.tipo, "tempoMedio": int(row.tempo_medio), "quantidade": row.quantidade} for row in rows]

    def obter_dados_comissao(self, filtros: Optional[Dict]) -> List[Dict]:
        stmt = select(
            func.coalesce(ProposicaoModel.orgao_atual, "Desconhecido").label("comissao"),
            func.count().label("quantidade"),
            func.coalesce(func.avg(ProposicaoModel.tempo_total_dias), 0).label("tempo_medio")
        ).group_by(func.coalesce(ProposicaoModel.orgao_atual, "Desconhecido"))
        
        stmt = self._aplicar_filtros(stmt, filtros)
        stmt = stmt.order_by(func.avg(ProposicaoModel.tempo_total_dias).desc()).limit(10)
        
        rows = self.session.exec(stmt).all()
        return [{"comissao": row.comissao, "tempoMedio": int(row.tempo_medio), "quantidade": row.quantidade} for row in rows]
    ...
    def obter_dados_status(self, filtros: Optional[Dict]) -> List[Dict]:
        ...
        rows = self.session.exec(stmt).all()
        return [{"status": row.status, "quantidade": row.quantidade, "percentual": int((row.quantidade / total) * 100) if total > 0 else 0} for row in rows]
    ...
    def obter_gargalos(self, filtros: Optional[Dict]) -> List[Dict]:
        stmt = select(
            ...
            func.count().label("quantidade"),
            func.sum(case((and_(ProposicaoModel.tempo_total_dias > 180, ProposicaoModel.data_encerramento.is_(None)), 1), else_=0)).label("atrasos"),
            func.coalesce(func.avg(ProposicaoModel.tempo_total_dias), 0).label("tempo_medio")
        ).group_by(func.coalesce(ProposicaoModel.orgao_atual, "Desconhecido"))
        ...
        resultado = []
        for row in rows:
            taxa_atraso = (row.atrasos / row.quantidade * 100) if row.quantidade > 0 else 0
    ...
    def obter_proposicoes_para_temas(self, filtros: Optional[Dict]) -> List[Dict]:
        ...
        stmt = self._aplicar_filtros(stmt, filtros)
        rows = self.session.exec(stmt).all()
        return [{"tags": row.tags, "tempo_total_dias": row.tempo_total_dias, "status_agrupado": row.status_agrupado} for row in rows]
