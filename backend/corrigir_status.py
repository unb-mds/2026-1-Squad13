...
def corrigir_status():
    try:
        with Session(engine) as session:
            ...
            print(f"Encontradas {len(models)} proposições para correção no banco de dados.")
            ...
            for model in models:
                ...
                if entity.status != status_anterior or model.status_original != entity.status_original:
            ...
            if corrigidas > 0:
                session.commit()
                print(f"Sucesso: {corrigidas} proposições foram corrigidas e normalizadas no banco de dados local!")
            else:
