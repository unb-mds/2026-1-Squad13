...
2. **Inicialização do Banco**:
   - Utilize o script `scripts/run-db-init.sh` para reconstruir o banco local de desenvolvimento e testar a aplicação de DDLs (Data Definition Language) do SQLModel.
...
3. **Validação do Seed de Produção/Demo**:
   ...
   - Utilize o script `scripts/validate-seed.sh` para certificar-se de que a injeção inicial de dados (seja estática ou conectando-se a dados mockados das APIs) ocorre sem erros.
...
## Scripts da Skill
- `scripts/run-db-init.sh`: Executa `init_db.py` para recriar as tabelas.
- `scripts/validate-seed.sh`: Executa `seed.py` para alimentar tabelas adicionais e validar consistência.
