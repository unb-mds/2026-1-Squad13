...
2. **Inicialização do Banco (Asíncrona)**:
   ...
   - **Gerenciamento de Tarefas:** Se a inicialização demorar ou travar o terminal, execute o processo em segundo plano (background) e monitore o seu progresso através da ferramenta `manage_task` do Antigravity CLI.
...
4. **Idempotência e Teste de Reset**:
   ...
   - Sempre que alterar os scripts de persistência, execute um teste de ciclo de vida completo localmente: Reconstrua $\rightarrow$ Aplique Seed $\rightarrow$ Re-aplique Seed $\rightarrow$ Verifique integridade e ausência de duplicidade nas tabelas principais.
