...
| Tempo de tramitação      | O sistema apresenta o tempo total de tramitação, tempo por fase e status de atraso baseado no IAR. | Alta       | O tempo total e status de atraso aparecem para a proposição; o sistema exibe o IAF para a fase atual e o IEI. |
| Métricas analíticas      | O sistema calcula e exibe métricas como IAR mediano, percentual de atrasadas e IEI médio.                     | Alta       | O dashboard apresenta agregações de atraso; os valores mudam de acordo com os filtros aplicados.              |
...
*   **Normalização Analítica:** Os eventos brutos de tramitação são transformados em `EventoTramitacao`, classificados por `TipoEvento` e associados a uma `FaseAnalitica` para permitir cálculos de duração por etapa (`dias_na_etapa`).
*   **Normalização de Status:** Os diversos status de tramitação retornados pelas APIs serão mapeados para uma taxonomia interna padronizada.
