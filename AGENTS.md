...
5. **Análise e Validação Prévia Obrigatória:** O agente de IA não deve realizar alterações funcionais de código, refatorações, correções de bugs complexos ou criação de novos arquivos sem antes apresentar um planejamento ou análise prévia ao desenvolvedor. Modificações simples de estilo, formatação e linting (como execução do `ruff format` ou pequenos ajustes de estilo triviais) são exceções e podem ser aplicadas diretamente sem planejamento prévio. Qualquer modificação de lógica ou estrutura física na árvore de trabalho (`working tree`) do repositório permanece estritamente condicionada à validação e autorização explícita do usuário.
